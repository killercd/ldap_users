...
class CustomFormatter(argparse.ArgumentDefaultsHelpFormatter,
    ...
    """Combines default values display with nicely formatted examples."""
    pass
...
def scan_hosts(ip_file,user, password, use_ssl):

    tls_config =Tls(validate=ssl.CERT_NONE, version=ssl.PROTOCOL_TLSv1_2)
    ldap_ip_file = open(ip_file, 'r')
    for ip in ldap_ip_file.readlines():
        ...
        try:
            ...
            ldap_search(conn, base_domain, '(sAMAccountName=*)')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DC Users Enumeration Tool",
        formatter_class=CustomFormatter,
        epilog=examples
    )
    parser.add_argument("-f", "--file", required=True, help="File containing list of DC IPs")
    ...
    scan_hosts(args.file, user, password, args.use_ssl)
